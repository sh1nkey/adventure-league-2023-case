...
def get_timetable(user_profile, if_even):
    days_with_subj_times = list(
        Day.objects.filter(week__group=user_profile.group, week__even_or_uneven=if_even)
        .prefetch_related("part_of_timetable")
        .values(
            "day_of_the_week",
            "part_of_timetable__subject__name",
            "part_of_timetable__pair_number",
        )
    )
    ...
    for entry in days_with_subj_times:
        ...
        day_schedule[day_of_the_week].append(
            {"subject": subject_name, "time": pair_number}
        )

    result = []
    for day, subjects in day_schedule.items():
        result.append(
            {"day_of_the_week": day, "subjects": subjects if subjects else None}
        )
    return result
